			...
			case "auth":
				...
				req.on("end", () => {
					if (search) {
						...
						if (!login.found) {
							res.writeHead(401);
							content = `<h1>${res.statusCode.toString()} ${res.statusMessage.toString()}</h1><p>User '${login.username}' does not exist in this system. Did you mean to <a href="/signup?username=${login.username}">sign up</a>?</p>`;
							res.write(templateHTML.replace("<!-- content -->", content));
							return res.end();
						}
					} else {
